...
public class SparkKafkaStreamWordCount implements Serializable {
	...
	public void run(Boolean groupByKey) {
		...
		JavaDStream<Tuple2<String, String>> wordCount;
		if (groupByKey) {
			wordCount = getWordCountByKey(messages);
		} else {
		...
		JavaDStreamKafkaWriter writer = JavaDStreamKafkaWriterFactory
				.getKafkaWriter(wordCount, props,
						PropertiesStack.getResultKafkaTopic(),
						PropertiesStack.isKafkaAsync());
		writer.writeToKafka();
	...
	private JavaDStream<Tuple2<String, String>> getWordCount(
		...
		JavaDStream<Tuple2<String, String>> wordCount = messages
				...
				.map(new Function<Tuple2<String, Integer>, Tuple2<String, String>>() {
					...
					public Tuple2<String, String> call(
						...
						return new Tuple2<String, String>(null, tuple._1()
								+ "\t" + tuple._2());
					}
				});
		return wordCount;
	...
	private JavaDStream<Tuple2<String, String>> getWordCountByKey(
			JavaPairInputDStream<String, String> messages) {
		JavaDStream<Tuple2<String, String>> wordCount = messages
				.groupByKeyAndWindow(Durations.seconds(10)).flatMap(
						new SparkGroupedLinesFlatMapFunction());
		return wordCount;
	}
